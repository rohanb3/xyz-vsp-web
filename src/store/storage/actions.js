...
export default {
  ...
  async [LOAD_TENANT_MINIFIED_COMPANIES]({ commit, getters }, data) {
    if (!Object.keys(getters.tenantCompanies).length) {
      const companies = await getMinifiedCompaniesByTenants(data);
      commit(SET_TENANT_MINIFIED_COMPANIES, companies);
      return companies;
    }

    return getters.tenantCompanies;
  },
  async [LOAD_TENANT_MINIFIED_USERS]({ commit, getters }, data) {
    if (!Object.keys(getters.tenantUsers).length) {
      const users = await getMinifiedUsersByTenants(data);
      commit(SET_TENANT_MINIFIED_USERS, users);
      return users;
    }

    return getters.tenantUsers;
  },
  async [LOAD_FULL_DEVICES_LIST]({ commit, getters }) {
    if (!Object.keys(getters.allDevices).length) {
